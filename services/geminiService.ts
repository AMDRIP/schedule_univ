...
export const generateScheduleWithGemini = async (data: GenerationData): Promise<ScheduleEntry[]> => {
  ...
  const prompt = `
    ...
    8.  АУДИТОРИИ: Вместимость аудитории ('capacity') должна быть не меньше количества студентов ('studentCount'). Для каждой дисциплины в поле 'classroomTypeRequirements' указаны подходящие типы аудиторий для КАЖДОГО ТИПА ЗАНЯТИЯ (лекция, практика и т.д.). Например, \`{ "Лекция": ["ct-lektsionnaya"], "Практика": ["ct-computer"] }\`. Это СТРОГОЕ требование.
